...
<div class="container pt-2">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('cybers.create') }}" class="btn btn-primary add-btn rounded-pill px-4 py-2">
            <i class="bi bi-plus-circle me-1"></i> Add New Section
        </a>
    </div>

    @if(session('success'))
        <div class="alert alert-success shadow-sm rounded-3" style="background: var(--success-color); color: #ffffff; border: none;">{{ session('success') }}</div>
    @endif
                ...
                <table class="table align-middle table-hover mb-0">
                    <thead style="background: linear-gradient(135deg, var(--primary-color), var(--secondary-color)); color: #ffffff; border: 2px solid var(--primary-color);">
                        <tr>
                            ...
                            <th class="text-center py-3" style="font-size: 0.875rem; font-weight: 600;">Subtitle</th>
                            <th class="text-center py-3" style="font-size: 0.875rem; font-weight: 600;">Image</th>
                            <th class="text-center py-3" style="font-size: 0.875rem; font-weight: 600;">Frame</th>
                            <th class="text-center py-3" style="font-size: 0.875rem; font-weight: 600;">Status</th>
                            <th class="text-center py-3" style="font-size: 0.875rem; font-weight: 600; width: 150px;">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($cybers as $key => $cyber)
                        <tr class="text-center" style="transition: background 0.2s ease;">
                            <td style="font-size: 0.875rem; color: var(--text-primary);">{{ $key + 1 }}</td>
                            <td class="fw-semibold" style="font-size: 0.875rem; color: var(--text-primary);">{{ $cyber->title }}</td>
                            <td style="font-size: 0.875rem; color: var(--text-secondary);">{{ $cyber->subtitle }}</td>
                            <td>
                                @if($cyber->image)
                                    <img src="{{ asset('storage/' . $cyber->image) }}" 
                                         alt="Cyber Image" 
                                         class="img-thumbnail rounded shadow-sm" 
                                         style="width: 80px; height: 60px; border: none; transition: transform 0.2s ease;">
                                @else
                            ...
                            <td>
                                @if($cyber->frame_image)
                                    <img src="{{ asset('storage/' . $cyber->frame_image) }}" 
                                         alt="Frame Image" 
                                         class="img-thumbnail rounded shadow-sm" 
                                         style="width: 80px; height: 60px; border: none; transition: transform 0.2s ease;">
                                @else
                                    <span class="text-muted fst-italic" style="font-size: 0.75rem;">No Frame</span>
                                @endif
                            </td>
                            <td>
                                @if($cyber->is_active)
                            ...
                            <td>
                                <div class="d-flex justify-content-center gap-2">
                                    <a href="{{ route('cybers.edit', $cyber->id) }}" 
                                    ...
                                    <form action="{{ route('cybers.destroy', $cyber->id) }}" 
                                          method="POST" 
