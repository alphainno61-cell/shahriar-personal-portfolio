...
<div class="pt-2">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('travels.create') }}" class="btn btn-primary add-btn rounded-pill px-4 py-2">
                ...
                <table class="table align-middle table-hover mb-0">
                    <thead style="background: linear-gradient(135deg, var(--primary-color), var(--secondary-color)); color: #ffffff; border: 2px solid var(--primary-color);">
                        <tr>
                            ...
                            <th class="text-center py-3" style="font-size: 0.875rem; font-weight: 600;">Country Name</th>
                            <th class="text-center py-3" style="font-size: 0.875rem; font-weight: 600;">Flag</th>
                            <th class="text-center py-3" style="font-size: 0.875rem; font-weight: 600;">Map Image</th>
                    ...
                    <tbody>
                        ...
                        <tr class="text-center" style="transition: background 0.2s ease;">
                            <td style="font-size: 0.875rem; color: var(--text-primary);">{{ $travel->id }}</td>
                            <td class="fw-semibold" style="font-size: 0.875rem; color: var(--text-primary);">{{ $travel->country_name }}</td>
                            <td>
                                @if($travel->country_flag_path)
                                    <img src="{{ asset('storage/'.$travel->country_flag_path) }}" 
                                         alt="Country Flag" 
                                         width="50" 
                                         class="img-thumbnail rounded shadow-sm" 
                                         style="border: none; transition: transform 0.2s ease;">
                                @else
                                    <span class="text-muted fst-italic" style="font-size: 0.75rem;">No Flag</span>
                                @endif
                            </td>
                            <td>
                                @if($travel->map_image_path)
                                    <img src="{{ asset('storage/'.$travel->map_image_path) }}" 
                                         alt="Map Image" 
                                         width="60" 
                                         class="img-thumbnail rounded shadow-sm" 
                                         style="border: none; transition: transform 0.2s ease;">
                                @else
                                    <span class="text-muted fst-italic" style="font-size: 0.75rem;">No Map</span>
                                @endif
                            </td>
                            <td>
                                @if($travel->is_active)
                            ...
                            <td>
                                <div class="d-flex justify-content-center gap-2">
                                    ...
                                    <form action="{{ route('travels.destroy', $travel->id) }}" 
                                          method="POST" 
                        ...
                        @empty
                        <tr>
                            <td colspan="6" class="text-center text-muted py-4" style="font-size: 0.875rem; color: var(--text-secondary);">No countries found.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
